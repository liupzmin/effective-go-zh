...
Go 的 switch 在功能上要比 C 更加广泛。其表达式不拘于常量或者整数，分支自顶下下求值，直至遇到匹配项，而且如果 switch 没有表达式，则分支以 true 为条件进行求值。因此在 Go 中可以使用 switch 来代替 if-else-if-else 链，实际上使用 switch 更加地道。
